...
from schemas.acao import AcaoCreate
from schemas.fii import FiiCreate
from repositories.acao import list_acoes_all, create_acao
from repositories.fii import list_fiis_all, create_fii
...
def update_all(db: Session):
    ...
    for modalidade in modalidades:
        ...
        for _, row in df.iterrows():
            if modalidade == 'acao':
                acao = AcaoCreate(**row.to_dict())
                create_acao(db, acao)
            elif modalidade == 'fii':
                fii = FiiCreate(**row.to_dict())
                create_fii(db, fii)
            else:
                raise ValueError("Invalid modalidade. Use 'acao' or 'fii'.")
